...
lensOperators =
  ...
  , "(#%=) :: MonadState s m => ALens s s a b -> (a -> b) -> m Unit"
  , "(#%~) :: ALens s t a b -> (a -> b) -> s -> t"
  , "(#=) :: MonadState s m => ALens s s a b -> b -> m Unit"
  , "(#~) :: ALens s t a b -> b -> s -> t"
  ...
  , "(%=) :: MonadState s m => ASetter s s a b -> (a -> b) -> m Unit"
  , "(%@=) :: MonadState s m => AnIndexedSetter i s s a b -> (i -> a -> b) -> m Unit"
  , "(%@~) :: AnIndexedSetter i s t a b -> (i -> a -> b) -> s -> t"
  , "(%~) :: ASetter s t a b -> (a -> b) -> s -> t"
  , "(&) :: a -> (a -> b) -> b"
  , "(&&=) :: MonadState s m => ASetter' s Bool -> Bool -> m Unit"
  , "(&&~) :: ASetter s t Bool Bool -> Bool -> s -> t"
  , "(&~) :: s -> State s a -> s"
  , "(**=) :: (MonadState s m, Floating a) => ASetter' s a -> a -> m Unit"
  , "(**~) :: Floating a => ASetter s t a a -> a -> s -> t"
  , "(*=) :: (MonadState s m, Num a) => ASetter' s a -> a -> m Unit"
  , "(*~) :: Num a => ASetter s t a a -> a -> s -> t"
  , "(+=) :: (MonadState s m, Num a) => ASetter' s a -> a -> m Unit"
  , "(+~) :: Num a => ASetter s t a a -> a -> s -> t"
  , "(-=) :: (MonadState s m, Num a) => ASetter' s a -> a -> m Unit"
  , "(-~) :: Num a => ASetter s t a a -> a -> s -> t"
  , "(.=) :: MonadState s m => ASetter s s a b -> b -> m Unit"
  , "(.>) :: (st -> r) -> (kab -> st) -> kab -> r"
  , "(.@=) :: MonadState s m => AnIndexedSetter i s s a b -> (i -> b) -> m Unit"
  , "(.@~) :: AnIndexedSetter i s t a b -> (i -> b) -> s -> t"
  , "(.~) :: ASetter s t a b -> b -> s -> t"
  , "(//=) :: (MonadState s m, Fractional a) => ASetter' s a -> a -> m Unit"
  ...
  , "(<>=) :: (MonadState s m, Semigroup a) => ASetter' s a -> a -> m Unit"
  ...
  , "(<||=) :: MonadState s m => LensLike' (Tuple Bool) s Bool -> Bool -> m Bool"
  , "(<||~) :: LensLike (Tuple Bool) s t Bool Bool -> Bool -> s -> Tuple Bool t"
  , "(<~) :: MonadState s m => ASetter s s a b -> m b -> m Unit"
  , "(?=) :: MonadState s m => ASetter s s a (Maybe b) -> b -> m Unit"
  , "(??) :: Functor f => f (a -> b) -> a -> f b"
  , "(?~) :: ASetter s t a (Maybe b) -> b -> s -> t"
  , "(^#) :: s -> ALens s t a b -> a"
  , "(^.) :: s -> Getting a s a -> a"
  , "(^..) :: s -> Getting (Endo [a]) s a -> [a]"
  , "(^=) :: (MonadState s m, Num a, Integral e) => ASetter' s a -> e -> m Unit"
  ...
  , "(^^=) :: (MonadState s m, Fractional a, Integral e) => ASetter' s a -> e -> m Unit"
